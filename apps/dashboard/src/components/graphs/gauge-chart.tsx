...
export default function GaugeChart({
	...
	const validatedProgress = adjustedProgress < 0 ? 0 : adjustedProgress > 100 ? 100 : adjustedProgress;

	// Calculate the stroke-dashoffset for the progress circle considering the gap
	const strokeDashoffsetProgress = circumference - (validatedProgress / 100) * (circumference - gap);

	const strokeColor = progressClassName || textColorSchema[color];
			...
			<div className="relative" style={{ width: size }}>
				...
				>
					...
					<circle
						...
						cy={size / 2}
						stroke="currentColor"
						className={cn("duration-500", strokeColor)}
