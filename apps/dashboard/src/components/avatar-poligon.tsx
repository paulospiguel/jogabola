...
export default function AvatarPoligon({
	...
	borderColor,
	label,
}: { className?: string; borderColor?: string; avatarImage: string; label: string }) {
	const clipPolygon = avatarImage
		? "[clip-path:polygon(50%_0,_100%_25%,_100%_75%,_50%_100%,_0_75%,_0_25%)] rounded-none"
		: "rounded-full";

	return (
		<Avatar className={cn(borderColor, clipPolygon, "p-0.5 flex items-center justify-center", className)}>
