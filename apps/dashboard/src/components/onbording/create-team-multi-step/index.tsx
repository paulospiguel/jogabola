...
export default function CreateTeamMultiStep() {
	const { STORAGE_KEY, methods: form, isClient, goToStep, currentStep: step } = useCreateTeam();
	...
	const onSubmit: SubmitHandler<z.infer<typeof teamSchema>> = async (values) => {
		try {
			...
			form.reset();
			localStorage.removeItem(STORAGE_KEY);
			result?.data?.slug ? push(`/manager/teams/${result?.data?.slug}`) : goToStep(1);
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : "Ocorreu um erro ao criar o time";
	...
	return (
		<Card className="w-full max-w-full mx-auto bg-white">
			<CardHeader>
				<CardTitle>Criar Time de Futebol</CardTitle>
				<CardDescription>Preencha as informações do seu time em 3 etapas simples.</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="mb-6">
			...
			<CardFooter className="flex justify-between">
				...
				{step < 3 ? (
					<Button disabled={isDisabledNextStep()} type="button" onClick={nextStep} className="ml-auto">
						Próximo
					</Button>
				) : (
