...
import { CalendarIcon, Trophy, Users, Dumbbell, Handshake, MapPin, Plus, Save, PlusIcon, Trash } from "@repo/ui/icons";
import { cn } from "@repo/ui/lib/cn";
import { AddOccasionalPlayer, type OccationalPlayer } from "./add-occasional-player";
	...
	const handlePlayerConvocation = (playerId: number | string) => {
		if (playerId === "all") {
			setMatchDetails((prev) => ({
				...prev,
				convokedPlayers: prev.convokedPlayers.length === players.length ? [] : players.map((player) => player.id),
			}));
			return;
		}
			...
			<div className="grid gap-4 py-4">
					...
					{isAddingNewTeam ? (
						...
						<div className="col-span-3 flex items-center space-x-2">
							<Select
								...
								onValueChange={(value) => setMatchDetails({ ...matchDetails, opponent: value })}
							>
					...
					<Popover>
						<PopoverTrigger asChild>
							...
							>
								...
								{matchDetails.date ? format(matchDetails.date, "PPP") : <span>Pick a date</span>}
							</Button>
						</PopoverTrigger>
						<PopoverContent className="w-auto p-0">
							<Calendar
								...
								onSelect={(date) => date && setMatchDetails({ ...matchDetails, date })}
								initialFocus
							/>
						</PopoverContent>
					</Popover>
				...
				<div className="grid grid-cols-4 items-center gap-4">
					...
					<RadioGroup
						...
						onValueChange={(value) => setMatchDetails({ ...matchDetails, matchType: value })}
						className="col-span-3"
					>
						<div className="flex items-center space-x-2">
							<RadioGroupItem value="competition" id="competition" />
							<Label htmlFor="competition">
								<Trophy className="h-4 w-4 inline mr-1" /> Competition
							</Label>
						</div>
						<div className="flex items-center space-x-2">
							<RadioGroupItem value="friendly" id="friendly" />
							<Label htmlFor="friendly">
								<Handshake className="h-4 w-4 inline mr-1" /> Friendly
							</Label>
						</div>
						<div className="flex items-center space-x-2">
							<RadioGroupItem value="training" id="training" />
							<Label htmlFor="training">
								<Dumbbell className="h-4 w-4 inline mr-1" /> Training
							</Label>
						</div>
						<div className="flex items-center space-x-2">
							<RadioGroupItem value="other" id="other" />
							<Label htmlFor="other">
								<Users className="h-4 w-4 inline mr-1" /> Other
							</Label>
						</div>
					</RadioGroup>
				</div>
				{matchDetails.matchType === "competition" && (
					<div className="grid grid-cols-4 items-center gap-4">
						...
						<Input
							...
							onChange={(e) => setMatchDetails({ ...matchDetails, competition: e.target.value })}
							className="col-span-3"
				...
				<div className="grid grid-cols-4 items-start gap-4">
					<Label className="text-right pt-2">Convoke Players</Label>
					<ScrollArea className="h-[200px] col-span-3 border rounded-md p-4">
						...
						<div className="flex items-center space-x-2">
							...
							<span className="bg-purple-500 h-2 w-2 rounded-full" />
							<span>Defender: 2/3</span>
							<span className="bg-teal-500 h-2 w-2 rounded-full" />
							<span>Forward: 2/3</span>
							<span className="bg-primary text-white rounded-full px-2">Convoked: 2/22</span>
						</div>
					</ScrollArea>
				</div>
			</div>
