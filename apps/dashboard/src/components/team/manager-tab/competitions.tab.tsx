...
import { Award, Check, Plus, PlusIcon, Save, Trash, Users } from "@repo/ui/icons";
...
import { FormControl, FormField } from "@repo/ui/components/form";
import { z } from "zod";
...
export default function CompetitionsTab({ hasEditPermission, session }: CompetitionsTabProps) {
	const [competitions, setCompetitions] = useState<z.infer<typeof competitionSchema>[]>(initialCompetitions);
	...
	const { register, handleSubmit, control, reset } = useForm<z.infer<typeof competitionSchema>>({
	...
	const handleSaveAdditionalInfo = (id: string) => {
		...
		setAdditionalInfoFields((prevState) => ({ ...prevState, [id]: existingField }));
	};
	...
	const handleUpdateAdditionalField = (id: string, label: string, value: string) => {
		const existingField = additionalInfoFields?.[id];
	...
	const handleSaveCompetition = (formData: z.infer<typeof competitionSchema>) => {
		...
		<>
			<Card>
				<CardHeader className="flex flex-row items-center justify-between">
					...
					{hasEditPermission && (
						<Button onClick={() => setIsCopetitionModalOpen(true)} disabled={isLoading}>
							<Plus className="h-4 w-4 mr-2" /> {t("tabs.competitions.addCompetition")}
						</Button>
					)}
				</CardHeader>
				<CardContent>
					<ul className="space-y-4">
						{competitions.map((competition) => (
							<li key={competition.id} className="bg-gray-50 p-4 rounded-lg border flex gap-4">
								...
								<div>
									<h3 className="font-semibold text-lg">{competition.competitionName}</h3>
									<p className="text-sm text-gray-600">{t(`tabs.competitions.${competition.type}`)}</p>
									<p className="text-sm text-gray-600">{competition?.startDate}</p>
									{competition.rank && (
										<p>
									...
									{competition.stage && (
										<p>
											{t("tabs.competitions.stage")}: {t(`tabs.competitions.${competition.stage}`)}
										</p>
									)}

									{Object?.entries(competition?.additionalInfo)?.map(([key, value]) => (
										<div key={key} className="flex">
											<span className="text-sm text-gray-600 mr-1 font-bold">{value.label}:</span>
			...
			<Dialog open={isCopetitionModalOpen} onOpenChange={onCloseAddCompetitionModal}>
					...
					<form onSubmit={handleSubmit(handleSaveCompetition)}>
						<div className="grid gap-4 py-4">
									...
									<FormField
										...
										render={({ field }) => (
											<Select onValueChange={field.onChange} defaultValue={field.value}>
												<SelectTrigger>
													<SelectValue placeholder={t("tabs.competitions.type")} />
												</SelectTrigger>
												<SelectContent>
								...
								<div className="space-y-2">
									...
									<FormField
										...
										render={({ field }) => (
											<Select onValueChange={field.onChange} defaultValue={field.value}>
												<SelectTrigger>
													<SelectValue placeholder={t("tabs.competitions.modality")} />
												</SelectTrigger>
												<SelectContent>
													{modality.map((item) => (
														<SelectItem key={item} value={item}>
							...
							<div>
								...
								<Button
									disabled={Object.values(additionalInfoFields || {})?.some((field) => !field.isSaved)}
									variant="outline"
									className="w-full my-2"
									onClick={() => handleAddAdditionalInfo(Date.now())}
								>
									<Plus className="h-4 w-4 mr-2" /> {t("tabs.competitions.additionalInfo")}
								</Button>

								<>
									{Object.entries(additionalInfoFields || {})?.map(([id, field]) => (
										<div key={id} className="grid grid-cols-5 gap-2 mb-2">
											<Input
												...
												onChange={(e) => handleUpdateAdditionalField(id, e.target.value, field.value)}
											/>
											<Input
												disabled={field.isSaved}
												id={field.value}
												...
												onChange={(e) => handleUpdateAdditionalField(id, field.label, e.target.value)}
											/>
											{!additionalInfoFields?.[id]?.isSaved && (
												<Button className="col-span-1" variant="default" onClick={() => handleSaveAdditionalInfo(id)}>
						...
						<DialogFooter className="flex justify-end">
							<Button>
								<Save className="h-4 w-4 mr-1" /> {t("global.save")}
							</Button>
						</DialogFooter>
					</form>
				</DialogContent>
