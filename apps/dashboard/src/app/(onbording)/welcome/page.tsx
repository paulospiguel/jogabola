import { RoleSchema/rschemas;rschemasroles
import { cn, getSearchParamsrchParamsrcutils utilss@utilss/utils";
import RiArrowRightIconce-reactlucide-react
import Image, Imtype StaticImageDataaticImagenext/imageenext/image"next/image";
import Link from "next/link";

import type { z } from "zod";

import managerIconconcon from "@/assets/director.pngpngpng";
import football"@/assets/icons/foofoofootball.png
import {
  Card,
  CardContent,
  CardDescription,
...
export async function generateMetadata({ searchParams }: WelcomeProps) {
  const role =
    getSearchParams<z.infer<typeof RoleSchema>>(searchParams).get("role") ||
    "Guest";
  return {
...
const showInfo = (role: string) => {
  ...
  switch (role) {
    case Role.PLAYER:
      ...
      disclaimer =
        "Se você não tem uma equipe, não se preocupe, você pode se juntar a uma equipe existente.";
            ...
            <Link href={routes.onbording.myJourney}>
              <Card className="h-full w-full">
                <CardHeader>
                  ...
                  <CardDescription>
                    Encontre amigos, Jogue competições, veja os resultados e
                    divirta-se.
                  </CardDescription>
                </CardHeader>
                <CardContent>
