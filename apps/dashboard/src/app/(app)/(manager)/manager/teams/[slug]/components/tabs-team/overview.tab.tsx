...
import { Calendar, MapPin, Users } from "@repo/ui/icons";
				...
				<div className="space-y-2">
					...
					<div className="flex items-center space-x-2">
						...
						<div className="flex items-center space-x-2">
							...
							<EditableInput initialValue={team?.manager || ""} onSave={(value) => handleSaveInfo("manager", value)} />
						</div>
					</div>
				</div>
				<div className="space-y-2">
					...
					<div className="flex items-center justify-start space-x-2">
						...
						<span className="font-bold">{team?.goalsFor - team?.goalsAgainst}</span>
					</div>
				</div>
			</div>
			<div className="mt-6">
				...
				<div className="space-y-2">
					<div className="flex items-center">
						...
						<Progress value={(team?.won / team?.played) * 100} className="flex-1" />
						<span className="ml-2 w-10 text-right">{team?.won}</span>
					</div>
					<div className="flex items-center">
						<span className="w-20">Drawn:</span>
						<Progress value={(team?.drawn / team?.played) * 100} className="flex-1" />
						<span className="ml-2 w-10 text-right">{team?.drawn}</span>
					</div>
					<div className="flex items-center">
						<span className="w-20">Lost:</span>
						<Progress value={(team?.lost / team?.played) * 100} className="flex-1" />
