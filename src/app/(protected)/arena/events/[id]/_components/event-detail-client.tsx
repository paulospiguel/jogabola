...
import { Calendar, Check, Clock, Link2, List, MapPin, Send, Trophy, X } from "lucide-react";
...
function ShareBar({ eventId, eventTitle }: { eventId: number; eventTitle: string }) {
  ...
  return (
    <div
      className="mb-4 rounded-[14px] border border-arena-border bg-arena-surface p-3.5"
    >
...
export function EventDetailClient({ event }: EventDetailClientProps) {
  ...
  const { confirmed, reserves, pending, isLoading } = useEventAttendance(event.id);
