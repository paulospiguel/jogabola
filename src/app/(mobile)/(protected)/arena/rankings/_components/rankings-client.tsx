    ...
    <div className="min-h-screen bg-arena-bg pb-24">
      ...
      {tab === "league" && (
        <div className="px-4 py-3.5">
          ...
          <motion.div className="flex flex-col" variants={stagger.container} initial="initial" animate="animate">
            {LIGA_STANDINGS.map((row, i) => (
              ...
              >
                <span className={cn("w-6 text-[12px] font-bold", row.pos <= 3 ? "text-arena-primary" : "text-arena-text-muted")}>
                  {row.pos}
                </span>
                <div className="flex flex-1 min-w-0 items-center gap-2">
                  <div className={cn("flex size-[22px] shrink-0 items-center justify-center rounded-[6px] border", row.own ? "border-arena-primary/44 bg-arena-primary/20" : "border-arena-border bg-arena-surface-el")}>
                    <span className={cn("text-[8px] font-extrabold", row.own ? "text-arena-primary" : "text-arena-text-muted")}>
                      {row.name.slice(0, 2).toUpperCase()}
                    </span>
                  </div>
                  <span className={cn("truncate text-[13px]", row.own ? "font-bold text-arena-primary" : "font-medium text-arena-text")}>
                    {row.name}
                  </span>
                  {row.own && (
                ...
                {[row.j, row.v, row.e, row.d].map((v, vi) => (
                  <span key={vi} className="w-7 text-center text-[12px] text-arena-text-sec">{v}</span>
                ))}
                <span className={cn("w-7 text-center text-[13px] font-bold", row.own ? "text-arena-primary" : "text-arena-text")}>
                  {row.pts}
                </span>
              </motion.div>
            ))}
      ...
      {tab === "scorers" && (
        <div className="flex flex-col gap-2 px-4 py-3.5">
          {TOP_SCORERS.map((p, i) => (
            ...
            >
              <span className={cn("w-6 text-[14px] font-extrabold", i === 0 ? "text-arena-highlight" : i < 3 ? "text-arena-primary" : "text-arena-text-muted")}>
                #{i + 1}
              </span>
              <JbAvatar name={p.name} size={34} />
              <div className="flex-1">
                <div className="text-[13px] font-semibold text-arena-text">{p.name}</div>
