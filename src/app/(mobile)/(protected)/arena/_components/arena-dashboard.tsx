...
type SheetType = "create-event" | "add-player" | "create-team" | null;
...
/**
 * Translates a `CockpitAction` (from `dashboard-cockpit.ts`) into the
 * concrete sheet/link it opens. The UI only maps action *types* here — it
 * never re-derives which action should show, that decision stays in the
 * pure view model.
 */
function CockpitActionButton({
  ...
  onOpenSheet: (sheet: "create-event" | "add-player") => void;
}) {
  ...
  switch (action.type) {
    case "create-event":
      return (
        <Cta
          variant={variant}
          size="md"
          onClick={() => onOpenSheet("create-event")}
        >
          <Plus size={15} />
          {t("cockpit.createEvent")}
        </Cta>
      );
    case "add-player":
      return (
        <Cta
          variant={variant}
          size="md"
          onClick={() => onOpenSheet("add-player")}
        >
          <Users size={15} />
          {t("cockpit.addPlayer")}
        </Cta>
    ...
    case "view-squad":
      return (
        <Cta
          variant={variant}
          size="md"
          onClick={() => router.push("/arena/squads")}
        >
          <Users size={15} />
          {t("cockpit.viewSquad")}
        </Cta>
    ...
    case "view-event":
      // Not rendered as a standalone CTA today — once there's an active
      // event, the hero card itself already links to it. Kept here so the
      // mapping stays exhaustive over `CockpitAction`.
      return (
        <Cta
          variant={variant}
          size="md"
          onClick={() => router.push(`/arena/events/${action.eventId}`)}
        >
          <Calendar size={15} />
          {t("hero.view")}
        </Cta>
...
export function ArenaDashboard({ userId }: ArenaDashboardProps) {
  ...
  const viewModel = buildDashboardViewModel({
    ...
    canManageTeam: activeTeamCanManage,
    events,
    squadCount: squad.length,
    now,
    isDiscoverFeatureActive: FEATURES.rankings || FEATURES.seasonHistory,
    // No per-card content source exists yet (e.g. rankings would need
    // computed standings) — every currently supported card is static, so
    // content is considered present whenever its feature flag is on.
    hasDiscoverContent: FEATURES.rankings || FEATURES.seasonHistory,
  });

  // The hero lets the captain flip through every eligible upcoming event,
  // starting from the cockpit's canonical pick. Sourced from the view
  // model (not the raw `events` array) so ordering is deterministic.
  const carouselEvents = viewModel.activeEvent
    ? [viewModel.activeEvent, ...viewModel.secondaryEvents]
    : [];

  const resolvedEventIndex = carouselEvents[activeEventIndex]
    ? activeEventIndex
    : 0;
  const activeEvent = carouselEvents[resolvedEventIndex] ?? null;
  ...
  const goToEvent = (direction: -1 | 1) => {
    ...
    setActiveEventIndex(current => {
      ...
      if (next < 0) return carouselEvents.length - 1;
      if (next >= carouselEvents.length) return 0;
      return next;
    });
  };
  ...
  // Only re-syncs when the carousel's size actually changes — intentionally
  // depends on `carouselEvents.length` rather than the array itself, which
  // is a new reference every render.
  useEffect(() => {
    setActiveEventIndex(current => {
      if (carouselEvents.length === 0) return 0;
      return current >= carouselEvents.length ? 0 : current;
    });
  ...
  // Structural placeholder for the true first load — events and squad
  // still fetching. The Arena layout keeps sidebar/topbar/bottom nav
  // mounted around this component regardless.
  if (eventsState.isInitialLoading || squadState.isInitialLoading) {
    return <DashboardSkeleton />;
  }

  const eventsViewState = deriveQueryViewState({
    hasData: viewModel.showMetrics,
    isInitialLoading: eventsState.isInitialLoading,
  ...
  const weekEvents = carouselEvents.filter(e => e.id !== activeEvent?.id);
  const cockpitActions = viewModel.actions;
          ...
          <div className="jb-dashboard-grid">
            <section className="jb-stack">
              {eventsViewState.status === "error" ? (
                <ArenaQueryError
                  title={t("cockpit.eventsErrorTitle")}
                  description={t("cockpit.eventsErrorDescription")}
                  retryLabel={t("cockpit.retry")}
                  onRetry={eventsState.refetch}
                  isRetrying={eventsState.isFetching}
                />
              ) : viewModel.showMetrics && activeEvent ? (
                <>
                      ...
                      <div className="flex items-center gap-2">
                        {hasMultipleEvents && (
                          <span className="rounded-lg border border-arena-border bg-arena-bg/35 px-2 py-[5px] text-[10px] font-bold text-arena-text-muted">
                            {eventPosition}/{carouselEvents.length}
                          </span>
                        )}
                  ...
                  <div className="jb-stat-grid">
                    {(
                      [
                        {
                          l: t("stats.confirmed"),
                          v: eventAttendanceLoading
                            ? "..."
                            : eventConfirmed.length,
                  ...
                  <Link href="/timer" className="jb-action justify-center">
                    <Timer size={14} />
                    {t("cockpit.openTimer")}
                  </Link>
                </>
                ...
                <ArenaEmptyState
                  ...
                  title={
                    activeTeamCanManage
                      ? t("cockpit.noEventManagerTitle")
                      : t("cockpit.noEventMemberTitle")
                  }
                  description={
                    activeTeamCanManage
                      ? t("cockpit.noEventManagerDescription")
                      : t("cockpit.noEventMemberDescription")
                  }
                  action={
                    cockpitActions && (
                      <div className="flex flex-wrap items-center justify-center gap-2">
                        <CockpitActionButton
                          action={cockpitActions.primary}
                          onOpenSheet={setSheet}
                          variant="primary"
                        />
                        {cockpitActions.secondary.map(secondaryAction => (
                          <CockpitActionButton
                            action={secondaryAction}
                            key={secondaryAction.type}
                            onOpenSheet={setSheet}
                            variant="secondary"
                          />
                        ))}
              ...
              <section>
                ...
                <div className="jb-stack">
                  {squadViewState.status === "error" ? (
                    <ArenaQueryError
                      title={t("cockpit.squadErrorTitle")}
                      description={t("cockpit.squadErrorDescription")}
                      retryLabel={t("cockpit.retry")}
