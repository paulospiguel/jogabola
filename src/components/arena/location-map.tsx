...
import { Check, Copy, ExternalLink, Loader2, MapPin, Pencil, Send, X } from "lucide-react";
...
export function LocationMap({ location: initialLocation, eventId, canEdit }: LocationMapProps) {
  ...
  async function handleSave() {
    ...
    if (res.success) {
      ...
      setEditError("Erro ao guardar. Tente novamente.");
    }
  }
          ...
          {loading ? (
              ...
              >
                <div className="flex flex-col items-center">
                  <div className="flex size-9 items-center justify-center rounded-full border-2 border-white bg-arena-primary shadow-lg">
                    <MapPin size={16} className="text-arena-bg" strokeWidth={2.5} />
                  </div>
            ...
            <div className="flex h-full items-center justify-center bg-arena-bg-sec">
              <div className="text-center">
                <MapPin size={28} className="mx-auto mb-2 text-arena-text-muted" strokeWidth={1.5} />
                <p className="text-[12px] text-arena-text-muted">Localização não encontrada</p>
              </div>
            </div>
          )}
        </div>
          ...
          {editing ? (
            <div className="mb-2.5">
              <div className="flex items-center gap-1.5 rounded-[10px] border border-arena-primary/50 bg-arena-bg px-3 py-2">
                ...
                />
                {saving ? (
                  <Loader2 size={14} className="shrink-0 animate-spin text-arena-primary" />
                ) : (
                  <>
                    <button
                      type="button"
              ...
              {editError && (
                <p className="mt-1 px-1 text-[11px] text-arena-danger">{editError}</p>
              )}
            </div>
          ) : (
            <div className="mb-2.5 flex items-start gap-2 rounded-[10px] border border-arena-border bg-arena-bg px-3 py-2.5">
              <MapPin size={14} className="mt-0.5 shrink-0 text-arena-primary" />
