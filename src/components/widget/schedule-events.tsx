...
function ScheduleEventsWidget({ date }: { date: CalendarEvent; hides: boolean }) {
	return (
		<div
			className={cn(
				"relative flex w-full items-center gap-2 overflow-hidden rounded-md pl-1 transition-all",
				date.bgcolor,
			)}
		>
...
export default function ScheduleEventWidget({ dates = testCalendarEventProps.dates }: CalendarEventProps) {
	const extraCount = dates?.length - maxEvents;
	return (
		<div className={cn("group relative flex size-full flex-col overflow-hidden rounded-3xl border-2 bg-white p-4")}>
			<div className="flex gap-1">
				<p className="text-xl font-bold text-red-400">{new Date().toLocaleString("default", { weekday: "short" })}</p>
			...
			<div className="my-2 flex flex-1 flex-col gap-2">
				{dates.slice(0, maxEvents).map((date, index) => (
					<ScheduleEventsWidget hides key={index} date={date} />
				))}
			</div>
			{extraCount ? (
				<>
